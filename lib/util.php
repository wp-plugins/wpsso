	...
	class WpssoUtil {
		...
		public function decode_utf8( $encoded ) {
			...
			$decoded = preg_replace( '/&#\d{2,5};/ue', 
				'WpssoUtil::decode_utf8_entity( \'$0\' )', $encoded );
		...
		public function cleanup_html_tags( $text, $strip_tags = true ) {
			...
			$text = preg_replace( '/<!--wpsso-ignore-->(.*?)<!--\/wpsso-ignore-->/i', ' ', $text);	// remove text between comment strings
			if ( $strip_tags == true ) 
		...
		public function parse_readme( $expire_secs = 0 ) {
			...
			if ( empty( $readme ) && $fh = @fopen( WPSSO_PLUGINDIR.'readme.txt', 'rb' ) ) {
				$use_local = true;
				$readme = fread( $fh, filesize( WPSSO_PLUGINDIR.'readme.txt' ) );
				fclose( $fh );
			}
		...
		public function get_source_id( $src_name, $atts = array() ) {
			...
			$source_id = $src_name.( empty( $atts['css_id'] ) ? 
				'' : '-'.preg_replace( '/^wpsso-/','', $atts['css_id'] ) );
